...
**首个注册用户自动成为管理员**，后续注册用户为普通用户。

### 需要管理员权限的操作

| 操作 | 接口 | 说明 |
|------|------|------|
| 插件上传/删除 | `/api/integration/plugins/*` | 防止恶意代码注入 |
| HA 连接配置 | `/api/ha/config` | 影响所有设备控制 |
| 模拟器开关 | `/api/simulator/*` | 影响演示设备 |
| 运维操作 | `/api/ops/*` | 诊断包、升级、备份 |
| 二级密码管理 | `/api/global/password/*` | 影响全局密钥 |
| 更新源配置 | `/api/ops/update/settings` | 影响升级来源 |

### 权限检查机制

- 请求到达时，中间件先验证 JWT 有效性
- 需要管理员权限的接口额外调用 `get_current_admin` 依赖注入
- 非管理员访问返回 403 `{"code": "admin_required", "message": "该操作需要管理员权限"}`

### 如何判断自己是否是管理员

调用 `GET /api/auth/me` 查看 `is_admin` 字段：

```jsonc
{
  "user_id": "uuid",
  "username": "alice",
  "is_admin": 1   // 1=管理员，0=普通用户
}
```
...
- **登出即撤销**：登出时把 token 的 `jti` 加入进程内黑名单，未过期的 token 也立即失效（防登出后凭证被复用）。黑名单存内存，重启清空——重启后旧 cookie 本来也会失效，语义一致。
...
> 注册接口的限流是为了防爆破。**第一个注册的用户自动成为管理员**（见上文「管理员分级」）。
