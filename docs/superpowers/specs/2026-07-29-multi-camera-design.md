...
- **关系**: 本方案把 `2026-07-29-onvif-camera-discovery-design.md`(单设备 ONVIF 发现,已实现)的发现能力整合进多路场景并参数化为 `camera_id`。根目录早期草稿《多摄像头改造方案.md》内容已并入本方案,该草稿已删除。
...
│  ▶ 基本信息                                      │
│    名称: [_____________]                         │
│    区域: [客厅 ▼]     ← HA /api/areas 下拉       │
│    来源: ● USB  ○ RTSP                           │
│    [来源相关字段...]     [测试连接]               │
│                                                  │
│  ▶ PTZ 云台                                      │
│    ☑ 启用 ONVIF                                   │
│    IP/端口/用户名/密码/速度/步进    [测试连接]     │
│                                                  │
│  ▶ ONVIF 自动发现                                │
│    ☑ 启用发现   device_mac: 60-a3-...(只读,自动) │
│    [重新发现]    手动填 IP: [____] [应用]         │
│                                                  │
│  ▶ 高级参数(折叠)                               │
│    运动检测: 哈希大小16  阈值15  间隔1.0s          │
│    视觉推理: 缩放宽448  JPEG质量60                │
│             最小间隔8.0s  最大空闲120.0s          │
│                                                  │
│  ▶ 视觉关注项                         [+ 添加]    │
│    [人] [宠物] [包裹] ...                        │
...
│  ▶ 自动化规则                         [+ 添加]    │
│    条件: 有人挥手   动作: 开灯    冷却: 5s         │
│    条件: 宠物出现   动作: 通知    冷却: 30s        │
│                                                  │
│    规则编辑使用与 ChatView 一致的 LLM 解析流程     │
└──────────────────────────────────────────────────┘
```

> 面板分区与各字段对应 §3.1 `cameras` 表列。区域下拉离线时降级为自由文本(见 §1.3 决策、§9 边界)。
...
├───────────────────────────────────────────────┤
│                                               │
│        <img src="video_feed" />               │  ← 随 activeCameraId 变化
│                                               │
├───────────────────────────────────────────────┤
│          ▲ (上)                                │
│     ◀    ●    ▶                               │  ← 当前摄像头的 PTZ
