...
也可以打开启动进度页 `http://localhost:8011/progress`（仅本机可访问）直接看。然后打开 `https://localhost:8010` 注册，表单里填这枚码——**第一个注册的用户自动成为管理员**。安装码只在没有用户时生效；怀疑泄露可在管理员登录后调 `POST /api/auth/setup-code/regenerate` 重置。

**给家人开通——邀请码**

之后的新账号一律凭管理员签发的一次性邀请码注册：**运维中心（/operations）→ 注册邀请码** → 生成，把码发给家人，对方在登录页切到注册模式填入即可。邀请码用一次即作废，可随时吊销。
