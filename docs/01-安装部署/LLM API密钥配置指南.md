...
这篇讲怎么配置 AI 大模型的"钥匙"（API 密钥）。Aether 把密钥管理统一放在 `/models` 页——LLM 的对话、视觉、嵌入、摘要、还有语音识别（STT），都用同一套机制。
...
STT 密钥也走 `/models` 页统一管理（type 选 `stt`）。配好后：
...
> 向导只配 4 个角色（不含 STT）。STT 之后在 `/models` 页单独加。

## 多用户隔离

Aether 支持多用户，每个用户的密钥是**独立隔离**的：

- 你在 `/models` 加的 Key 只属于你
- 切换用户时，会加载那个用户的密钥配置
...
| `chat` / `summary` / `stt` | per-user 优先；用户可在 `/models` 页切到"全局兜底"（`use_global` 开关） |

**入口**：`/models` 页 → 顶部 **「全局配置」** tab。
...
- 检查 API Key 有没有多余空格
- 确认网络能访问该服务商
- 去 `/models` 页用「测试模型」功能（`POST /api/models/test`）测对应角色的 key
