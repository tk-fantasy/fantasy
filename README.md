...
> LLM 密钥推荐用前端 `/keys` 页面管理，会自动写入 `.env` 并持久化到数据库。
...
侧边栏只有三个入口，其余功能通过斜杠命令或快捷链接到达：

| 入口 | 说明 |
|------|------|
| **管家** | 聊天主界面，输入 `/` 查看全部斜杠命令（设备、定时、监控、模型等一键跳转） |
| **设置** | 家庭信息、地区、深色模式 |
| **高级** | 弹窗式配置：天气 API、Exa 搜索、视觉参数（RTSP/运动检测/推理间隔）、Emoji 索引重建、文档向量重建，以及 HA 配置 / 助手角色 / API Keys 的快捷入口 |

> `/ha`、`/unique`、`/keys` 三个页面仍可直接通过 URL 访问，也收录在「高级」弹窗的快捷入口区。
