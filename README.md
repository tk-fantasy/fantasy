...
- **🌐 数据出网策略** —— 三档模式（云端对话/混合模式/纯内网），聊天页实时显示当前模式，满足不同安全需求
...
- `docs/03-设备控制/` —— 自然语言控制、设备控件、设备面板、**设备语义映射**、**AI 设备操作权限**
...
- `docs/08-运维排查/` —— API 鉴权、日志排查、健康检查、**运维中心**、数据出网策略
...
| **设置** | 家庭信息、地区、深色模式 |
| **高级** | 系统级配置页面：天气 API、Exa 搜索、视觉参数、HA 连接、助手角色、API Keys、虚拟设备开关、数据出网模式（点击卡片弹出 modal 编辑），以及 Emoji 索引重建、文档向量重建 |

**常用斜杠命令**（共 16 个，聊天框打 `/` 查看全部）：
