...
class TestSyncLlmKeysFiltersGlobalRoles:
    """测试 _sync_llm_keys_to_current_user 按 PER_USER_ROLES 过滤。

    vision/embed 全局共享，不该进 per-user DB；否则 wizard 跑完后
    全局 .env 丢失时运行时无法回退（embed_client 走全局解析），
    表现为 RAG/语义图/emoji 全部 401。
    """

    @pytest.mark.asyncio
    async def test_sync_filters_out_embed_and_vision(self):
        from app.routes.settings_routes import _sync_llm_keys_to_current_user

        global_keys = [
            {"id": "k1", "type": "chat", "base_url": "https://a.com", "model": "m1", "api_key": "c1"},
            {"id": "k2", "type": "embed", "base_url": "https://b.com", "model": "m2", "api_key": "e1"},
            {"id": "k3", "type": "vision", "base_url": "https://c.com", "model": "m3", "api_key": "v1"},
            {"id": "k4", "type": "summary", "base_url": "https://d.com", "model": "m4", "api_key": "s1"},
        ]
        stored_keys = {}
    ...
    @pytest.mark.asyncio
    async def test_sync_keeps_chat_summary_stt(self):
        from app.routes.settings_routes import _sync_llm_keys_to_current_user

        global_keys = [
            {"id": "k1", "type": "chat", "base_url": "https://a.com", "model": "m1", "api_key": "c1"},
            {"id": "k2", "type": "summary", "base_url": "https://b.com", "model": "m2", "api_key": "s1"},
            {"id": "k3", "type": "stt", "base_url": "https://c.com", "model": "m3", "api_key": "t1"},
        ]
        stored_keys = {}
