...
差异只有两点：role 与是否强制 _enabled=True。本工厂参数化这两个差异，
...
async def build_per_user_chat_client(
    ...
    Args:
        role: 客户端角色（chat）。automation/rule/scheduler/summarization 均用 chat
              （会话摘要复用对话模型，无独立 summary 角色）。
        ...
        force_enabled: 是否强制 _enabled=True。per-user 有 key 即启用、不受全局
            ...
            summarization 传 False，保留 enabled 开关语义。
