...
tests/                   # Backend pytest (1300+ tests)
...
```bash
# Backend
pytest                      # slow tests are deselected by default (see pytest.ini)
pytest -m slow              # run slow tests explicitly (real plugin subprocesses / RPC e2e, ~3 min)
...
CI runs `pytest` (which defaults to `not slow`) on every push and pull request via GitHub Actions.
...
> This project involves camera-frame analysis and home-device control. Please read the disclaimer template ([`docs/10-交付物料/免责声明模板.md`](docs/10-交付物料/免责声明模板.md), Chinese) before deployment.